...
import (
	...
	"github.com/AliyunContainerService/image-syncer/pkg/dockerhelper/rootless"
...
func builtinDefaultSignatureStorageDir(euid int) *url.URL {
	if euid != 0 {
		return &url.URL{Scheme: fileURLScheme, Path: filepath.Join(homedir.Get(), defaultUserDockerDir)}
	}
	return &url.URL{Scheme: fileURLScheme, Path: defaultDockerDir}
}
